...
def main():
    ...
    if args.mode == 'simple':
        evaluator = SimpleModelEvaluator(system_prompt=args.system_prompt)
        ...
        for result in results:
            ...
            code = result["response"].strip('`').replace('scala\n', '', 1) if result["response"].startswith('```') else result["response"]
        ...
        evaluator = PromptEvaluator(base_system_prompt=args.system_prompt)

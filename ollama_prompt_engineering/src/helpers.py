...
def save_final_report(total_successful_runs: int, total_snippets: int, variation_metrics: Dict, output_file: Path = None) -> None:
    if output_file is None:
        output_file = Path(__file__).parent.parent / "res" / "final_report.md"
    
    report_content = f"""# Evaluation Final Report
...
"""
    for variation_name, metrics in variation_metrics.items():
        ...
        report_content += f"""### {variation_name}
- Average BLEU Score: {avg_bleu:.4f}
- Success Rate: {success_rate:.2f}% ({metrics['successful_runs']}/{metrics['total_runs']} snippets)
